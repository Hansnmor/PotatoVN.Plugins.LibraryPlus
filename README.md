...
- **时长区间筛选**：<10h（短篇小品） / 10-20h（单线） / 20-40h（中等） / >40h（长篇） / 时长未知，与「排除已玩过」为 AND 关系（如：排除已玩过 + <10h = 找未玩过且几小时能通完的）；时长未知的游戏单独归入「时长未知」档（不在四个时长区间内）
...
3. 点击工具栏「排序」选择排序条件与方向；勾选「排除已玩过」或选择时长区间进行过滤；统计条跟随可见列表实时更新。
